...
class Judge:
    ...
    def evaluate(
        ...
        *,
        ...
        if e_value is None:
            return DecisionTrace(status="Invalid", reason="missing_e_value", e_value=None)
        ...
        threshold = 1.0 / self.policy.alpha
        if e_value >= threshold:
            return DecisionTrace(status="Supported", reason="e_value_pass", e_value=e_value)
        return DecisionTrace(status="Rejected", reason="e_value_fail", e_value=e_value)
